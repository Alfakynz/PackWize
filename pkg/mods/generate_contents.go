...
func GenerateContents(minecraftVersionArg, launcherArg, outputDir, nameArg string, quiet bool) {
	types := []struct {
		Key   string
		Label string
	}{
	...
	for _, minecraftVersion := range versions {
		for _, launcher := range launchers {
			...
			for _, t := range types {
				items := listProjects(minecraftVersion, t.Key, launcher, quiet)
				results[t.Key] = items
				if len(items) > 0 && !quiet {
...
func listProjects(minecraftVersion, projectType, launcher string, quiet bool) []string {
