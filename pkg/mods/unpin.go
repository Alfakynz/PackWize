...
// UnpinMod manages unpinning a mod for multiple versions and launchers
func UnpinMod(minecraftVersionArg, launcherArg, modsArg string) {
	// Convert arguments
	...
	for _, mod := range mods {
		...
		for _, v := range versions {
			for _, l := range launchers {
				fmt.Printf("Unpinning %s to %s/%s ...\n", mod, v, l)

				cmd := exec.Command("packwiz", "unpin", mod)
				cmd.Dir = fmt.Sprintf("%s/%s", v, l)
				cmd.Stdout = os.Stdout
				cmd.Stderr = os.Stderr

			if err := cmd.Run(); err != nil {
					log.Printf("Error unpinning %s to %s/%s: %v\n", mod, v, l, err)
				}
			}
		}
	}
